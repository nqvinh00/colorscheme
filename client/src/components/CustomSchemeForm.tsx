...
const CustomSchemeForm = ({
  ...
  onCancel,
  disableApi = false,
}: CustomSchemeFormProps) => {
  const isEdit = mode === "edit";
  const [isExpanded, setIsExpanded] = useState(isEdit ? true : false);
  const [formData, setFormData] = useState({
    name: initialScheme?.name || "",
    category: initialScheme?.category || "Custom",
    colors: initialScheme?.colors || {
      black: "#000000",
      red: "#ff0000",
      green: "#00ff00",
      yellow: "#ffff00",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // If not logged in, always create locally
    if (!token || !username) {
      ...
      if (onAddScheme) onAddScheme(newScheme);
      setFormData({
        name: "",
        category: "Custom",
        colors: {
    ...
    if (disableApi) {
      // Local creation: do NOT reference token or call API
      const newScheme: ColorScheme = {
        id: `custom-${Date.now()}`,
        name: formData.name,
        author: "Local User",
    ...
    // API logic below (requires valid token)
    if (isEdit && initialScheme) {
      // PUT to API for update
      const updatedScheme: ColorScheme = {
        ...initialScheme,
        name: formData.name,
        category: formData.category,
        colors: formData.colors,
      ...
      try {
        ...
        if (res.ok) {
          if (onUpdateScheme) onUpdateScheme(updatedScheme);
        } else {
          const data = await res.json().catch(() => ({}));
          setError(data.message || "Failed to update color scheme.");
        }
      } catch {
  ...
  return (
    <div className="mt-6 rounded-lg border border-gray-700 bg-gray-800 p-6">
      <h3 className="mb-4 text-xl font-semibold text-green-300">
        {isEdit ? "Edit Color Scheme" : "Create Custom Scheme"}
      </h3>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-2 pt-4">
          <Button
            ...
            disabled={loading}
          >
            {loading ? (isEdit ? "Saving..." : "Adding...") : isEdit ? "Save Changes" : "Add Scheme"}
          </Button>
          <Button
            type="button"
            variant="outline"
